...
#  print(edge_feature.shape)
    ...
    print("filter: ", net.shape)
#  print(filters)


    # 4 RESNET
    net = tf.matmul(net, net, transpose_a=True)  # shape=B*B
    print("matmul: ", net.shape)
    net = tf.reshape(net, [-1, 1024, 1024, 1])
    print("reshape[-1,1]: ", net.shape)
    n = 9
    net = tflearn.conv_2d(net, 256, 3, regularizer='L2', weight_decay=0.0001)
